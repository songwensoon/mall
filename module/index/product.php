...
html;
	...
	default:
		...
		$prorule_list_array = array();
		if(!empty($prorule_list)){
			foreach($prorule_list as $key=>$value){
				if(!empty($value['prorule_id'])){
					$prorule_id = explode(',',$value['prorule_id']);
					$prorule_list_array = array_merge($prorule_list_array,$prorule_id);
				}
			}
		}
		$rule_id_list = array();
		if(!empty($info['rule_id'])){
			$rule_id_list = explode(",",$info['rule_id']);
		}
		$rule_list = $db->pe_selectall('rule', '', '*', array());
		foreach ($rule_list as $k => $v) {
			if(in_array($v['rule_id'],$rule_id_list)){
				$rule_list[$k]['ruledata_list'] = $db->pe_selectall('ruledata', array('rule_id'=>$v['rule_id'],'ruledata_id'=>$prorule_list_array,'order by'=>'`ruledata_order` Asc'));
			}else{
				unset($rule_list[$k]);
			}
		}
