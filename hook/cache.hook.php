...
function cache_write($cache_type = 'all') {
	...
	switch ($cache_type) {
		...
		break;
		case 'rule':
			$rule_list = $db->index('rule_id')->pe_selectall('rule', array());
		    if(!empty($rule_list)){
				foreach($rule_list as $k=>$v){
					$ruledata = $db->index('ruledata_id')->pe_selectall('ruledata', array('order by'=>'`ruledata_order` asc','rule_id'=>$v['rule_id']));
					$rule_list[$k]['list'] = $ruledata;
				}
			}
